...
export const UsersStat: React.FC<UserStatProps> = memo(({ ageGroups, gengerGroups, totalUsers, loading }) => {
  const ageGroupsItems = useMemo(() => {
    return Object.entries(ageGroups).map(([range, value]) => <StatisticItem range={range} value={value} key={range} />);
  }, [ageGroups]);

  const gengerGroupsItems = useMemo(() => {
    return Object.entries(gengerGroups).map(([range, value]) => <StatisticItem range={range} value={value} key={range} />);
  }, [gengerGroups]);

  return (
    <div className={style.users_stat}>
      {loading ? <div className={style.sekelton_total_users}></div> : <p className={style.total_users}>{`${totalUsers} ${totalUsers === 1 ? 'user' : 'users'}`}</p>}
